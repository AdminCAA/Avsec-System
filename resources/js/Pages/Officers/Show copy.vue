...
const {personnel, qualifications,certifications, mandatorycourse, specialisetranining} = defineProps({
    personnel: {
        type: Object,
        required: true
    },
...
const paginatedControls = computed(() => {
  switch (activeTab.value) {
    ...
    case 'Specialised Course': return specialisetranining;    
    default: return [];
  }
});
...
const downloadPDF = async () => {
  try {
    ...
    const response = await axios.get(`/personnels/${personnel.id}/download`, {
      responseType: 'blob',
    });
    const url = window.URL.createObjectURL(new Blob([response.data]));
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `personnel_${personnel.name}.pdf`);
    ...
    <div class="content-wrapper">
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h3 class="m-0">Avsec Officers Details</h3>
            </div>
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item">
                  <Link class="btn btn-info" data-bs-toggle="tooltip" title="Edit"
                    :href="route('officers.edit',personnel.id)"><i class="fas fa-edit"></i> Edit</Link>
                </li>
                <li class="breadcrumb-item">
                ...
                <div class="card card-info card-outline">
                  <div class="card-body box-profile">
                    <div class="text-center">
                      <img class="profile-user-img img-fluid img-circle" style="width:120px" :src="personnel.portrait ? 
                    ...
                    <p class="text-muted text-center">Operator:
                      <Link :href="route('facilities.show',personnel.facility_id)">{{personnel.facility_name}}</Link>
                    </p>
                    ...
                    <hr>
                     <div class="d-flex justify-content-center mb-2">
                        <button class="btn btn-info btn-sm ms-2" @click="downloadPDF" :disabled="isDownloading">
                      <i class="fas fa-file-pdf" style="margin-right: 8px;"></i>
                      <span class="ms-1">Export to PDF</span>
                      <span v-if="isDownloading" class="spinner-border spinner-border-sm ms-2" role="status"></span>
                    </button>
                    </div>
                ...
                <div class="card card-info card-outline">
                  <div class="card-header p-2">
                    <ul class="nav nav-pills">
                      <li class="nav-item"><a class="nav-link active" href="#certifications" data-toggle="tab"><i
                            class="fas fa-shield-alt"></i> Certifications</a>
                      </li>

                      <li class="nav-item"><a class="nav-link" href="#qualifications" data-toggle="tab"><i
                            class="fas fa-user-graduate"></i> Qualifications</a>
                      </li>

                       <li class="nav-item"><a class="nav-link" href="#mandatorycourse" data-toggle="tab"><i
                            class="fas fa-exclamation-circle"></i> Mandatory Course</a>
                      </li>

                       <li class="nav-item"><a class="nav-link" href="#specialisetranining" data-toggle="tab"><i
                            class="fas fa-book-open"></i> Specialised Training</a>
                      </li>
                    ...
                    <div class="tab-content">
                      ...
                      <div class="active tab-pane" id="certifications">
                        ...
                        <div class="d-flex justify-content-end mb-2">
                          <Link :href="route('certifications.create', personnel.id)" data-bs-toggle="tooltip"
                            data-bs-placement="top" title="Add Certification" class="btn btn-info btn-sm "><i
                          ...
                          <table v-if="certifications.length > 0" id="example2"
                            ...
                            <tbody>
                              <tr v-for="(certificate, index) in certifications" :key="certificate.id"
                                ...
                                <td>
                                  {{ certificate.date_issued ? dayjs(certificate.date_issued).format('DD-MM-YYYY') :
                                  'N/A' }}
                                </td>
                                <td>
                                  {{ certificate.expiry_date ? dayjs(certificate.expiry_date).format('DD-MM-YYYY') :
                                  'N/A' }}
                                </td>
                                ...
                                <td>
                                  <div class="d-flex justify-content-end">
                                    <a v-if="certificate.certification_file" class="btn btn-success btn-sm mr-2"
                                      data-bs-toggle="tooltip" title="View Attachment"
                                      :href="`/storage/${certificate.certification_file}`" target="_blank">
                      ...
                      <div class="tab-pane" id="qualifications">
                        <div class="d-flex justify-content-end mb-2">
                          <Link :href="route('qualifications.create', personnel.id)" class="btn btn-info btn-sm "><i
                            class="fas fa-plus"></i> Add New</Link>
                        </div>
                            ...
                            <tbody>
                              <tr v-for="(qualification, index) in qualifications" :key="qualification.id"
                                ...
                                <td>
                                  ...
                                  dayjs(qualification.date_obtained).format('DD-MM-YYYY') : 'N/A' }}
                                </td>


                                <td>
                                  <div class="d-flex justify-content-end">
                                    <a v-if="qualification.qualification_file" class="btn btn-success btn-sm mr-2"
                                      data-bs-toggle="tooltip" title="View Attachment"
                                      :href="`/storage/${qualification.qualification_file}`" target="_blank">
                       ...
                       <!-- <div class="tab-pane" id="mandatorycourse">
                        <div class="d-flex justify-content-end mb-2">
                          <Link :href="route('qualifications.create', personnel.id)" class="btn btn-info btn-sm "><i
                            class="fas fa-plus"></i> Add New</Link>
                        </div>
                        ...
                        <div class="table-responsive">
                          <table v-if="mandatorycourse.length > 0" id="example2"
                            class="table table-sm table-bordered table-hover table-striped">
                            <thead>
                              <tr>
                                <th>#</th>
                            ...
                            <tbody>
                              <tr v-for="(qualification, index) in mandatorycourse" :key="qualification.id"
                                :class="{'table table-selected': selectedRowId2 === qualification.id }"
                                @click="selectRow2(qualification.id)">
                                <td>{{ index + 1 }}</td>
                                <td>
                          ...
                          <div v-else class="text-center mt-4">
                            <h5>No mandatory course found for this avsec officer</h5>
                          </div>
                        </div>
